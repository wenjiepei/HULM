function [ pred_label, probability ] = inference_hidden_unit_logistic_optimized( X, model )
...
%   [predicted_label] = inference_hidden_unit_logistic(X, model)
    ...
    [~, ~, log_M_temp(1, 1), ~, ~, ~, mid_psi_result] = forward_backward_hidden_unit_logistic_optimized_cmex(X, 1, model, 0, 0);

    for label=2:K
        [~, ~, log_M_temp(label, 1), ~, ~, ~, ~] = forward_backward_hidden_unit_logistic_optimized_cmex(X, label, model, 0, 1, mid_psi_result);
    end
