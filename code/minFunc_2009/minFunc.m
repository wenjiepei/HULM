...
% Output Log
if verboseI
%     disp(options.dataset_name);
    fprintf('%10s %10s %10s %15s %15s %15s %15s \n','Dataset', 'Iteration','FunEvals','Step Length','Function Val','Opt Cond', 'Elasped Time');
end
...
for i = 1:maxIter
    ...
    if verboseI
        time_e = clock;
        elapsed_t = etime(time_e, time_s);
%         disp(options.dataset_name);
        fprintf('%10s %10d %10d %15.5e %15.5e %15.5e %15.5f \n',options.dataset_name, i,funEvals*funEvalMultiplier,t,f,sum(abs(g)), elapsed_t);
    end
